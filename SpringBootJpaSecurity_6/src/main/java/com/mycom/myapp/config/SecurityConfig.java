...
//						
//				)
				// 위 .logout 은 get, post 모두 처리하지만, 모두 페이지 요청에 대한 처리
				// 백엔드는 페이지요청, 데이터요청을 기본적으로 구분 X
				// 현재 구현 코드 기준으로 get /logout 은 페이지 요청, post /logout 은 데이터 요청을 전제로 코드 변경
				.logout(logout -> logout
						.logoutRequestMatcher(new OrRequestMatcher(
								PathPatternRequestMatcher.pathPattern(HttpMethod.GET, "/logout"),
								PathPatternRequestMatcher.pathPattern(HttpMethod.POST, "/logout")))
						.logoutSuccessHandler( (request, response, authentication) -> {
							if("GET".equalsIgnoreCase(request.getMethod())) { // 페이지 요청의 logout
